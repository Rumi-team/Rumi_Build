...
**Priority:** P1 (SEO — live pages currently disown themselves)
This repo deploys as the Vercel project `rumi-ai` serving `https://www.rumiai.ai`; the twin repo `../Frontend_Rumi_AIEmployees` owns `rumi.build` (settled 2026-08-04 from both checkouts' `.vercel/project.json` plus `vercel ls`). But every canonical URL here still hardcodes `https://rumi.build`, so live rumiai.ai pages emit `<link rel="canonical" href="https://rumi.build/...">` — pointing search engines at the sibling site. Four root spots: `metadataBase` in `src/app/layout.tsx`, `BASE` in `src/app/sitemap.ts`, the sitemap URL in `src/app/robots.ts`, and every URL in `src/lib/llms-content.ts`. Per-page canonicals are relative and resolve through `metadataBase`, so those four are the whole swap; `routing.test.ts` and `crawler-surfaces.test.ts` both derive the expected host from `metadataBase` rather than hardcoding it, so a consistent swap keeps them green (they exist to catch a *partial* swap). Separately: `SUPPORT_EMAIL` in `src/lib/data.ts` and the `support@rumi.build` addresses in the footer, Terms, Privacy and `/book/success` are a different call — that mailbox presumably exists, an @rumiai.ai one may not.

**Presented with this evidence on 2026-08-05 and deliberately deferred** — CLAUDE.md requires confirming before changing a canonical URL, and the answer was hold off. Until it lands, `/team`'s meta description (changed in v1.2.0.1) is the one user-visible surface naming rumiai.ai while its own canonical says rumi.build. Do not re-propose without new information. CLAUDE.md's "Nothing local says which domain this deploys to" section is stale and should be rewritten in the same change.
